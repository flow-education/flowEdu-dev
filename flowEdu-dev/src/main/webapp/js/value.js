...
function itemTypeName(itemCategory, itemType) {
	...
	if (itemCategory != "" && itemType != "") {
		...
		else if (itemCategory == "3" && itemType == "4") str = "스페셜 의상";
		else if (itemCategory == "4" && itemType == "1") str = "지붕";
		else if (itemCategory == "4" && itemType == "2") str = "벽지";
		else if (itemCategory == "4" && itemType == "3") str = "스페셜 둥지";
		else if (itemCategory == "5" && itemType == "1") str = "숲";
		else if (itemCategory == "5" && itemType == "2") str = "스페셜 숲";
	}
	return str;
}
...
function getReserveCancelStatus(idx, status, sendStatus, sendDate) {
	var str = "";
	var now = new Date();
	var unixDate = convertUnixTime(sendDate);

	if (status == "0" ) {
		if (now < unixDate) {
			str = "<a href=\"javascript:deleteConfirm(" + "'" + idx + "')\" class='button compact red-gradient glossy'><span class='icon-forbidden'>발송취소</span></a>";	
	...
	/*
	if (status == "0" && sendStatus == "0") {
		if (now > unixDate) {
			str = "발송안됨";
		} else {
			str = "<a href=\"javascript:deleteConfirm(" + "'" + idx + "')\" class='button compact red-gradient glossy'><span class='icon_cancel'>발송취소</span></a>";	
			
		}
	} else if (status == "1" && sendStatus == "1") {
		if (now > unixDate) {
			str = "발송완료";
		} else {
			str = "취소불가";	
		}
	} else if (status == "1" && sendStatus == "0") {
		str = "취소불가";
	}
	*/
	return str;
...
function reservePushStatus(reserveStatus, pushInfoStatus, reserveDate) {
	var status = "";
	var now = new Date();
	var unixReserveDate = convertUnixTime(reserveDate);
	
	if (reserveStatus == "0" && pushInfoStatus == "0") status = "예약중";
	else if (reserveStatus == "1" && pushInfoStatus == "0") status = "발송대기";
	else if (reserveStatus == "1" && pushInfoStatus == "1") status = "발송완료";
	
	return status;
}

function getLetterPaperCategoryName(category) {
	var name = "";
	if (category == 1) {
		name = "스페셜";
	} else if (category == 2) {
